CC = gcc
CFLAGS = -g -Wall -Wextra -std=c99 -fPIC -MMD -MP
LDFLAGS = -shared
...
clean:
	...
	rm -f $(LINENOISE_OBJ)
	rm -rf query/build
	rm -f query/*.o query/*.d query/*.gcov query/*.gcda query/*.gcno
	rm -f query/deps/*.o query/deps/*.d
...
# Automatic header dependency files (generated by -MMD -MP): touching a header
# rebuilds every object that includes it, preventing stale-enum bugs.
-include $(LIB_OBJECTS:.o=.d) query/deps/linenoise.d \
        query/build/csvql.d query/build/test_grammar.d query/build/test_query.d
