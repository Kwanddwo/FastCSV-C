...
int main(void) {
    ...
    QueryResult r1 = query_execute(config, "SELECT * FROM 'query/data/students.csv';", &arena);
    if (r1.error) {
    ...
    QueryResult r2 = query_execute(config, "SELECT name, age FROM 'query/data/students.csv';", &arena);
    if (r2.error) {
    ...
    printf("\n=== Test 3: Parse error ===\n");
    QueryResult r3 = query_execute(config, "SELECTX * FROM 'query/data/students.csv';", &arena);
    if (r3.error) {
        printf("Error: %s (line %d, col %d)\n", r3.error, r3.error_line, r3.error_column);
    }

    printf("\n=== Test 4: Column not found ===\n");
    QueryResult r4 = query_execute(config, "SELECT FakeCol FROM 'query/data/students.csv';", &arena);
    if (r4.error) {
        printf("Error: %s\n", r4.error);
    }
